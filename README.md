...
```bash
lambdaimdb$ sam logs -n HelloEfsFunction --stack-name lambdaimdb --tail
```
...
```bash
lambdaimdb$ pip install -r tests/requirements.txt --user
# unit test
lambdaimdb$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
lambdaimdb$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name lambdaimdb
```
